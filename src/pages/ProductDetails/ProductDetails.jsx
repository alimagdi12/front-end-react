...
const placeholderImage =
  "https://upload.wikimedia.org/wikipedia/commons/3/3f/Placeholder_view_vector.svg"; // Define the path to your placeholder image
export default function ProductDetails() {
  const { color } = useContext(ColorContext);
  const { products } = useContext(ProductsContext);
  const { id } = useParams();
  const { cartItems, deleteCartItem, addToCart, getCart } =
  ...
  const getImage = (images, index) =>
    images && images[index] ? images[index] : placeholderImage;
        ...
        <Box sx={{ width: "75%", padding: "50px", marginX: "auto" }}>
          <Grid container spacing={5}>
              ...
              >
                <li>
                  <label htmlFor="slide-1">
                    <img
                      src={getImage(product?.imagesUrl?.images, 0)}
                      alt="Product Image 1"
                    />
                  </label>
                </li>
                ...
                <li>
                  <label htmlFor="slide-3">
                    <img
                      src={getImage(product?.imagesUrl?.images, 2)}
                      alt="Product Image 3"
                    />
                  </label>
                </li>
            ...
            <Grid item xs={12} md={5}>
              ...
              <Box sx={{ display: "flex", my: 2 }}>{/* <HoverRating /> */}</Box>
              <Typography margin={1} sx={{ textAlign: "left" }}>
                Publisher: {product?.userId?.firstName}{" "}
              ...
              <Link to={`/chat/${product?.userId?._id}`}>
                <Button
                  ...
                  sx={{
                    ...
                    "&:hover": { color: "black", backgroundColor: "#FAAF00" },
                  }}
                >
                  Chat With Seller
                </Button>
              ...
              <Button
                variant="contained"
                onClick={toggleDrawer("right", true, product._id)}
                sx={{
                  backgroundColor: color,
                  "&:hover": { color: "black", backgroundColor: "#FAAF00" },
                }}
              ...
              >
                ...
                {cartItems?.map((item) => (
                  <Paper
                    ...
                    sx={{ p: 2, marginBottom: "0px" }}
                  >
                    <Grid container spacing={2} alignItems="center">
                      <Grid item xs={12} md={2} height="100px">
                        <Avatar
                          variant="square"
                          src={getImage(item.productId?.imagesUrl.images, 0)}
                          sx={{
                            width: "100%",
                            height: "100%",
                            borderRadius: "5px",
